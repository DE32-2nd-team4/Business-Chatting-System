...
def chatbot(message):
        ...
        for keywords in apidict[keys]:
            if keywords == keyword:
                ...
                if len(movie_cd_json['movieListResult']['movieList']) != 1:
                    ...
                    global_command = [f"{bot_nic}님, 정확히 어떤 영화를 찾으시나요? 번호를 입력해주세요. ex) @bot 숫자 \n\n{movies_info_list}", search_word, command, movie_cd_list]
    ...
    if len(movie_cd) > 0:
        ...
        if command == 'genreNm':
            global_command = [f"{bot_nic}님, 영화 \"{search_word}\"의 장르는 {movie_info['genres'][0]['genreNm']}입니다."]
        elif command == 'directors_peopleNm':
            global_command = [f"{bot_nic}님, 영화 \"{search_word}\"의 감독은 {movie_info['directors'][0]['peopleNm']}입니다."]
        elif command == 'actors_peopleNm':
            actors = movie_info['actors']
            if len(actors) > 10:
                actors = actors[:5]
            actor_list = '\n'.join([f"{actor['peopleNm']} ({actor['cast']} 역)" for actor in actors])
            global_command = [f"{bot_nic}님, 영화 \"{search_word}\"의 배우는 {actor_list}입니다."]
        elif command == 'openStartDt':
            global_command = [f"{bot_nic}님, 영화 \"{search_word}\"의 개봉일은 {movie_info['openDt']}입니다."]
        elif command == 'movieNm':
            global_command = [f"{bot_nic}님, 영화의 제목은 \"{search_word}\"입니다."]
    else:
        global_command = ["{bot_nic}님, 해당 영화는 없습니다."]
...
def chatbotFindMovie(to_bot_data2):
    ...
    if command == 'genreNm':
        ...
        global_command = [f"{bot_nic}님, 영화 \"{find_name}\"의 장르는 {genre_list}입니다."]
    elif command == 'directors_peopleNm':
        directors = find_movie['directors']
        director_list = ', '.join([f"{director['peopleNm']}" for director in directors])
        global_command = [f"{bot_nic}님, 영화 \"{find_name}\"의 감독은 {director_list}입니다."]
    elif command == 'actors_peopleNm':
...
# chat.log 파일 터치 (존재하지 않을 경우 생성)
    ...
    while(True):
        message = global_command[0]  # 챗봇 메시지 전송
        m_message = {'nickname': 'bot4', 'message': message, 'time':time.time()}
